...
export class InfoBar extends Lightning.Component {
  ...
  _construct() {
    ...
    xhr.open(
      'GET',
      window.serverdata.SERVER_URL + 'OemLogo?operator_id=' + window.serverdata.OPERATOR_ID
    )
    xhr.responseType = 'json'
    xhr.onload = () => {
      const data = xhr.response
      if (data[0] && data[0].Logo) {
        this.tag('Logo').patch({ LogoflexItem: { src: Utils.asset(data[0].Logo) } })
      }
    }
    xhr.send()
  }
  ...
  geticonstatus() {
    this.tag('ThunderNetworkService')
      .getnetworkstatus()
      .then(network => {
        if (network === 'Wifi') {
    ...
    this.tag('ThunderBluetoothService')
      .getbluetoothstatus()
      .then(bluetooth => {
        if (bluetooth === 'true') {
