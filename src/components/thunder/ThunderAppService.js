...
export class ThunderAppService extends Lightning.Component {
  ...
  async launchPremiumApp(data) {
    ...
    if (data.title == 'Youtube') {
      ...
      if (
        ...
        let launchType = data.type
        if (launchType !== 'dial') {
          await this.configureAppLanguage('Cobalt', langUrl)
        }
  ...
  /* Method to launch an application with a particular language url
   * @param {*} plugin
   */
  ...
  launchNewLang(type) {
    ...
    let newurl = lang[type] || lang['default']
    return newurl
  }
  ...
  closePlugin() {
    if (this.activeApp == 'Cobalt') {
      ...
      this.thunderJS
        .call('org.rdk.RDKShell', 'suspend', { callsign: 'Cobalt' })
        .then(() => {
    ...
    if (this.activeApp == 'Netflix') {
      ...
      this.thunderJS
        .call('org.rdk.RDKShell', 'suspend', { callsign: 'Netflix' })
        .then(() => {
    ...
    if (this.activeApp == 'Amazon') {
      ...
      this.thunderJS
        .call('org.rdk.RDKShell', 'suspend', { callsign: 'Amazon' })
        .then(() => {
    ...
    this.thunderJS.call(
      ...
      (err, result) => {
        if (err) {
          ...
          let highest_resolution =
            Storage.get('lastsetresolution') || display_array[display_array_length - 1]
          Log.info('\n Display settings get result length' + display_array_length)
          Log.info('\n Display settings get highest resolution' + highest_resolution)
          this.thunderJS.call(
            'org.rdk.DisplaySettings',
            'setCurrentResolution',
            { videoDisplay: 'HDMI0', resolution: highest_resolution, persist: true },
            (err, result) => {
              if (err) {
                Log.info('\n Display settings set error')
                this.thunderJS.call('org.rdk.DisplaySettings', 'setCurrentResolution', {
                  videoDisplay: 'HDMI0',
                  resolution: '    1080p60',
                  persist: true
                })
  ...
  moveToBack(clientName) {
    this.thunderJS
      .call('org.rdk.RDKShell', 'moveToBack', { client: clientName })
      .then(result => {
  ...
  /**
   * ConfigureApplication plugin.
   *
   * Used to configure the plugin whose callsign is
   * passed as string
   *
   * @param_values string: callsign of the plugin
   * @return bool: if the activation process as successfull or not.
   */

  async configureApplication(appName, config_data) {
    let full_url = config_data.url + '&inApp=true'
    this._activate(appName)
    setTimeout(() => {
      return new Promise((resolve, reject) => {
        this.thunderJS
          .call('Cobalt', 'deeplink', { full_url })
          .then(resp => {
            resolve(true)
          })
          .catch(err => {
   ...
   * ConfigureApplication plugin.
   *
   * Used to Method to configure the App URL for the callsign is
   * passed as string
   * passed as string
   *
   * @param_values string: callsign of the plugin
  ...
  configureAppLanguage(callsign, config_lang = '') {
    let plugin = 'Controller'
    let method = 'configuration@' + callsign
    return new Promise(resolve => {
      this.thunderJS.call(plugin, method).then(res => {
        res.language = config_lang
        this.thunderJS
          .call(plugin, method, res)
          .then(resp => {
            console.log('Ttag:Configure application success' + JSON.stringify(resp))
            resolve()
          })
          .catch(err => {
            console.log('Ttag:error' + JSON.stringify(err))
            resolve()
          })
      })
    })
