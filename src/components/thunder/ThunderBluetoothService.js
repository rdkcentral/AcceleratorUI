...
export class ThunderBluetoothService extends Lightning.Component {
  ...
  _construct() {
    ...
    this.thunderJS.on('org.rdk.Bluetooth', 'onStatusChanged', notification => {
      ...
      if (notification.newStatus === 'DISCOVERY_COMPLETED') {
        this.fireAncestors("$_showDevices","Discovered")        
      }
    })
  }
  ...
  async getDevices() {
    ...
    await this.wait_promise(1000)
    window.devices = this.uniqDevices(discovered.concat(paired), item => item.deviceID)
    Log.info('Devices Paired/Discovered are' + JSON.stringify(window.devices))
  }
