...
export class ThunderDiagnosticService extends Lightning.Component {
  ...
  _diagnostic() {
    ...
    return new Promise(function(resolve, reject) {
      diagObj.thunderJS.call('DeviceInfo', 'systeminfo', (err, result) => {
        if (err) {
          Log.info('\n Diagnostic error')
          reject()
        } else {
  ...
  _diagnosticVersion() {
    let diagObj = this
    return new Promise(function(resolve, reject) {
      diagObj.thunderJS.call('org.rdk.System', 'getSystemVersions', (err, result) => {
        if (err) {
      ...
      diagObj.thunderJS.call(
        ...
        (err, result) => {
          if (err) {
            ...
            Log.info(
              '\n Dtag: Display settings getCurrentResolution(result.resolution) DiagnosticResolutionLabel',
              diagResl
            )
            resolve(diagResl)
          }
